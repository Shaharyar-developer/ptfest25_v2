...
function MotionHighlight<T extends string>({
  ...
  const parentModeProps =
    mode === "parent" ? (props as ParentModeMotionHighlightProps) : undefined;
  const normalizedBoundsOffset = React.useMemo<Bounds>(
    () => ({
      top: parentModeProps?.boundsOffset?.top ?? 0,
      left: parentModeProps?.boundsOffset?.left ?? 0,
      width: parentModeProps?.boundsOffset?.width ?? 0,
      height: parentModeProps?.boundsOffset?.height ?? 0,
    }),
    [
      parentModeProps?.boundsOffset?.top,
      parentModeProps?.boundsOffset?.left,
      parentModeProps?.boundsOffset?.width,
      parentModeProps?.boundsOffset?.height,
    ]
  );
  const parentContainerClassName = parentModeProps?.containerClassName;
  const parentForceUpdateBounds = parentModeProps?.forceUpdateBounds;
  const itemsClassName = (props as { itemsClassName?: string }).itemsClassName;
  ...
  const safeSetBounds = React.useCallback(
    (bounds: DOMRect) => {
      ...
      const newBounds: Bounds = {
        top: bounds.top - containerRect.top + normalizedBoundsOffset.top,
        left: bounds.left - containerRect.left + normalizedBoundsOffset.left,
        width: bounds.width + normalizedBoundsOffset.width,
        height: bounds.height + normalizedBoundsOffset.height,
      };
      ...
      if (mode === "parent") {
        return (
          <div
            ...
            className={cn("relative", parentContainerClassName)}
          >
  ...
  const contextValue = React.useMemo(
    () => ({
      ...
      forceUpdateBounds: parentForceUpdateBounds,
    }),
    [
      mode,
      activeValue,
